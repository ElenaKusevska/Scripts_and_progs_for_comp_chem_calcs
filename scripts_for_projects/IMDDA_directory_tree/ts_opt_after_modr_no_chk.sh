      ...
      do
         ...
         if [ -d "$dirs" ]; then
            ...
            if [ -d "$i" ]; then
               ...
               if [[ $j == "DMF" ]]; then
                  echo '#p opt(ts,calcfc,noeigen) freq 5d '$k'/6-31+g(d) scrf(smd,solvent=n,n-DiMethylFormamide) gfinput' | cat >> temp
               elif [[ $j == "gas" ]]; then
                  echo '#p opt(ts,calcfc,noeigen) freq 5d '$k'/6-31+g(d) gfinput' | cat >> temp
               elif [[ $j == "o-DCB" ]]; then
                  echo '#p opt(ts,calcfc,noeigen) freq 5d '$k'/6-31+g(d) scrf(smd,solvent=o-DiChloroBenzene) gfinput' | cat >> temp
               fi
               ...
               sed -n '/^0\|^1\|^-1/ {p;q}' $i.g16 | cat >> temp 
                        # just the line with charge and multiplicity
                              sed -n 'H; /Standard orientation/h; ${g;p;}' $i.out | sed -n '/Standard orientation/,/Rotational/p' | sed -n '/1/,/----/p' | sed -n '/-------------/q;p' | cut -c 17-28,32-95 | sed -n 's/^17 / Cl /g;p' | sed -n 's/^ 6 / C /g;p' | sed -n 's/^14 /Si /g;p' | sed -n 's/^ 1 / H /g;p' | sed -n 's/^ 7 / N /g;p' | sed -n 's/^ 8 / O /g;p' | sed -n 's/^16 / S /g;p' | cat >> temp
