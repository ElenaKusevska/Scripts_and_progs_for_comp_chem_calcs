   ...
   do
      ...
      do
         ...
         if [ -d "$i" ]; then
            ...
            if [[ $j == "DMF" ]]; then
               echo '#p opt(ts,calcfc,noeigen) freq 5d '$k'/6-31+g(d) scrf(smd,solvent=n,n-DiMethylFormamide) Geom=Checkpoint NoFreeze gfinput' | cat >> temp
            elif [[ $j == "gas" ]]; then
               echo '#p opt(ts,calcfc,noeigen) freq 5d '$k'/6-31+g(d) Geom=Checkpoint NoFreeze gfinput' | cat >> temp
            elif [[ $j == "o-DCB" ]]; then
               echo '#p opt(ts,calcfc,noeigen) freq 5d '$k'/6-31+g(d) scrf(smd,solvent=o-DiChloroBenzene) Geom=Checkpoint NoFreeze gfinput' | cat >> temp
            fi
