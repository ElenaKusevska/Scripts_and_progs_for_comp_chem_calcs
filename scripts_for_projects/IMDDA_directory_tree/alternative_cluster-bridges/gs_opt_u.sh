      ...
      do
         ...
         then
            ...
            exit
  			...
  			echo %NProc=$np | cat >> $i.com
  			echo %Mem=$mg'GB' | cat >> $i.com
  			echo %chk=$i.chk | cat >> $i.com

			if [[ $j == "DMF" ]]; then
            echo '#p opt freq=noraman 5d u'$k'/6-31+g(d) scrf(smd,solvent=n,n-DiMethylFormamide) guess=(mix,always) gfinput' | cat >> $i.com
			elif [[ $j == "gas" ]]; then
            echo '#p opt freq=noraman 5d u'$k'/6-31+g(d) guess=(mix,always) gfinput' | cat >> $i.com
         elif [[ $j == "o-DCB" ]]; then
            echo '#p opt freq=noraman 5d u'$k'/6-31+g(d) scrf(smd,solvent=o-DiChloroBenzene) guess=(mix,always) gfinput' | cat >> $i.com
         fi
