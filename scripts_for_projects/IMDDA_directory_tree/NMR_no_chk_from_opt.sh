   ...
   do
      ...
      do
         ...
         if [ -d "$i" ]; then
            ...
            if [[ $j == "DMF" ]]; then
               echo '#p NMR 5d '$k'/6-311+g(d,p) scrf(smd,solvent=n,n-DiMethylFormamide) gfinput' | cat >> temp
            elif [[ $j == "gas" ]]; then
               echo '#p NMR 5d '$k'/6-311+g(d,p) gfinput' | cat >> temp
            elif [[ $j == "o-DCB" ]]; then
               echo '#p NMR 5d '$k'/6-311+g(d,p) scrf(smd,solvent=o-DiChloroBenzene) gfinput' | cat >> temp
            fi
            ...
            mv temp $i.g16
            mv $i.out $i-opt.log
            rm -f $i.slurm $i.fchk $i.xyz $i.gjf $i.out $i.chk output
