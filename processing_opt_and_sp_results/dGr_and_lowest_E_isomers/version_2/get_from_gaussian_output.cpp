...
int gauss_output_one_isomer(std::string chemical_species, double& E_opt, \
      ...
      double& S_sp, double& T, int& imaginary_freq, \
   ...
   // Get the SCF energy:
   get_gout_value("python -m goodvibes -f 0 -t 363 *.out | grep 'o '", 
         E_opt, 2);

   std::this_thread::sleep_for (std::chrono::seconds(1));

   // get the enthalpy:
   get_gout_value("python -m goodvibes -f 0 -t 363 *.out | grep 'o '", H_opt, 4) ;
   
   std::this_thread::sleep_for (std::chrono::seconds(1));

   // Get the Gibbs free energy:
   get_gout_value("python -m goodvibes -f 0 -t 363 *.out | grep 'o '", G_opt, 7) ;
...
int sort_multiple_geometries (std::vector<std::string> isomers, \
   ...
   for (i=0; i<isomers.size(); i=i+1) {
      ...
      // Get the Gibbs free energy
      get_gout_value("python -m goodvibes -f 0 -t 363 *.out | grep 'o '", G_opt_sort[i], 7) ;
...
int gauss_output_multiple_isomers(std::string chemical_species, \
   ...
   double& S_sp, double& T, double& cpu_time, double& E_sp, double& G_sp, \
