   ...
   do
      if [ -d "$(pwd)/$i" ]; then
         ...
         if [ -f "$(pwd)/$i.out" ]; then
            ...
            zero_point=$(grep 'Sum of electronic and zero-point Energies=' *.out | tail -1 | cut -c54-65)

            enthalpy=$(grep 'Sum of electronic and thermal Enthalpies=' *out | tail -1 | cut -c54-65)
