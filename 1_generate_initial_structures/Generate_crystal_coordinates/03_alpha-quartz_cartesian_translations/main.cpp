...
// The translations are performed in the Cartrsian coordinate
...
// used as input for MBNexplorer calculation.
//
...
int main() {
   ...
   int i, j, k, ipm, jpm, kpm, atom, natoms;
   double dist, sum;
   ...
   std::vector<double> new_H_coordinates;
   ...
   // lattice vectors:
   e1 = {4.2522, -2.455, 0.0}; // x-axis
   e2 = {0.0, 4.91, 0.0}; // y-axis
   e3 = {0.0, 0.0, 5.402}; // z-axis
   ...
   n1 = int( (d1/4.91)/2.0 + 10.0 );
   n2 = int( (d2/4.91)/2.0 + 10.0 );
   n3 = int( (d3/5.402)/2.0 + 10.0 );
                  ...
                  for (kpm=0; kpm<=1; kpm=kpm+1) {
                     ...
                     for (atom=0; atom<basis_atoms.size(); atom=atom+1) {
                        ...
                        atom_coordinates.push_back(basis_atoms[atom][0]+
                           e1[0]*pm[ipm]*double(i) + e2[0]*pm[jpm]*double(j) +
			   e3[0]*pm[kpm]*double(k));
                        atom_coordinates.push_back(basis_atoms[atom][1] +
                           e1[1]*pm[ipm]*double(i) + e2[1]*pm[jpm]*double(j) +
			   e3[1]*pm[kpm]*double(k));
                        atom_coordinates.push_back(basis_atoms[atom][2] +
                           e1[2]*pm[ipm]*double(i) + e2[2]*pm[jpm]*double(j) +
			   e3[2]*pm[kpm]*double(k));
