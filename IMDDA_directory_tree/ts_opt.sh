   ...
   do
      ...
      do
         ...
         cp $gjffile $i'.gjf'
			...
			if [[ $j == "DMF" ]]; then
  				echo '#p opt=(calcfc,ts,noeigen) freq=noraman 5d '$k'/6-31+g(d) scrf(smd,solvent=n,n-DiMethylFormamide) gfinput' | cat >> $i.g16
			elif [[ $j == "gas" ]]; then
            echo '#p opt=(calcfc,ts,noeigen) freq=noraman 5d '$k'/6-31+g(d) gfinput' | cat >> $i.g16
         elif [[ $j == "o-DCB" ]]; then
            echo '#p opt=(calcfc,ts,noeigen) freq=noraman 5d '$k'/6-31+g(d)scrf(smd,solvent=o-DiChloroBenzene) gfinput' | cat >> $i.g16
         fi
