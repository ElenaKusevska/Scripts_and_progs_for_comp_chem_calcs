   ...
   do
      ...
      do
         ...
         if [ -d "$i" ]; then
            ...
            if [[ $j == "DMF" ]]; then
               echo '#p opt(ts,calcfc,noeigen) freq 5d u'$k'/6-31+g(d) scrf(smd,solvent=n,n-DiMethylFormamide) guess=mix Geom=Checkpoint gfinput' | cat >> temp
            elif [[ $j == "gas" ]]; then
               echo '#p opt(ts,calcfc,noeigen) freq 5d u'$k'/6-31+g(d) guess=mix Geom=Checkpoint gfinput' | cat >> temp
            elif [[ $j == "o-DCB" ]]; then
               echo '#p opt(ts,calcfc,noeigen) freq 5d u'$k'/6-31+g(d) scrf(smd,solvent=o-DiChloroBenzene) guess=mix Geom=Checkpoint gfinput' | cat >> temp
            fi
