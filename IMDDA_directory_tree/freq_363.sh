      ...
      do
         ...
         if [ -d "$dirs" ]; then
            ...
            if [ -d "$i" ]; then
               ...
               if [[ $j == "DMF" ]]; then
                  echo '#p freq 5d '$k'/6-31+g(d) scrf(smd,solvent=n,n-DiMethylFormamide) Geom=Checkpoint pop=nbo Temperature=363 output=wfn gfinput' | cat >> temp
               elif [[ $j == "gas" ]]; then
                  echo '#p freq 5d '$k'/6-31+g(d) Geom=Checkpoint pop=nbo Temperature=363 output=wfn gfinput' | cat >> temp
               elif [[ $j == "o-DCB" ]]; then
                  echo '#p freq 5d '$k'/6-31+g(d) scrf(smd,solvent=o-DiChloroBenzene) Geom=Checkpoint pop=nbo Temperature=363 output=wfn gfinput' | cat >> temp
               fi
               ...
               echo  $j'_'$k'/'$i ' - does not exist' $(pwd)
               # exit
               cd ..
               sleep 5
            fi
