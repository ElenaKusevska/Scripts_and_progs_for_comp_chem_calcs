   ...
   do
      ...
      do
         ...
         if [ -d "$i" ]; then
            ...
            if [[ $j == "DMF" ]]; then
               echo '#p BOMD(Phase=('$pa1','$pa2'),RTemp=300,MaxPoints=350) 5d u'$k'/6-31+g(d) scrf(smd,solvent=n,n-DiMethylFormamide) guess=(mix,always)' | cat >> temp
            elif [[ $j == "gas" ]]; then
               echo '#p BOMD(Phase=('$pa1','$pa2'),RTemp=300,MaxPoints=350) 5d u'$k'/6-31+g(d) guess=(mix,always)' | cat >> temp
            elif [[ $j == "o-DCB" ]]; then
               echo '#p BOMD(Phase=('$pa1','$pa2'),RTemp=300,MaxPoints=350) 5d u'$k'/6-31+g(d) scrf(smd,solvent=o-DiChloroBenzene) guess=(mix,always)' | cat >> temp
            fi
