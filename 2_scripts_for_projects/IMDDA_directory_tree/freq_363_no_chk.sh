      ...
      do
         ...
         if [ -d "$dirs" ]; then
            ...
            if [ -d "$i" ]; then
               ...
               if [[ $j == "DMF" ]]; then
                  echo '#p freq 5d '$k'/6-31+g(d) scrf(smd,solvent=n,n-DiMethylFormamide) Temperature=363 gfinput' | cat >> temp
               elif [[ $j == "gas" ]]; then
                  echo '#p freq 5d '$k'/6-31+g(d) Temperature=363 gfinput' | cat >> temp
               elif [[ $j == "o-DCB" ]]; then
                  echo '#p freq 5d '$k'/6-31+g(d) scrf(smd,solvent=o-DiChloroBenzene) Temperature=363 gfinput' | cat >> temp
               fi
