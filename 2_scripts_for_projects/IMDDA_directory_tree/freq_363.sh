      ...
      do
         ...
         if [ -d "$dirs" ]; then
            ...
            if [ -d "$i" ]; then
               ...
               hr=9 # projected run time of job
               ...
               if [[ $j == "DMF" ]]; then
                  echo '#p freq 5d '$k'/6-31+g(d) scrf(smd,solvent=n,n-DiMethylFormamide) Geom=Checkpoint Temperature=363 gfinput' | cat >> temp
               elif [[ $j == "gas" ]]; then
                  echo '#p freq 5d '$k'/6-31+g(d) Geom=Checkpoint Temperature=363 gfinput' | cat >> temp
               elif [[ $j == "o-DCB" ]]; then
                  echo '#p freq 5d '$k'/6-31+g(d) scrf(smd,solvent=o-DiChloroBenzene) Geom=Checkpoint Temperature=363 gfinput' | cat >> temp
               fi
