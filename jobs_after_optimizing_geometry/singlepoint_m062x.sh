...
time=12:00:00 # expected runtime of job
...
do
  ...
  echo '#p 5d m062x/6-311+g(d,p) scrf(smd,solvent=n,n-DiMethylFormamide) Geom=Checkpoint pop=nbo output=wfn gfinput' | cat >> temp
  echo ' ' | cat >> temp
  echo $i | cat >> temp
  echo ' ' | cat >> temp
  ...
  echo '#SBATCH --time='$time | cat >> $i.slurm
